...
Поскольку ValkarnTasks подключается непосредственно к этому циклу, `await VlkTask.Yield()` не блокирует поток — он регистрирует коллбэк, который Unity вызывает при следующем тике выбранной фазы, и немедленно возвращает управление.
...
Перед встраиванием `PlayerLoopHelper` проверяет, присутствует ли `VlkTaskUpdate` где-либо в текущем дереве PlayerLoop. Если да, встраивание пропускается. Это делает встраивание **идемпотентным** — многократный вызов `Init()` (что может произойти в редакторе) никогда не приводит к регистрации дублирующихся систем.
...
Когда вы делаете `await VlkTask.Yield()` (или что-то, что приостанавливается ровно на один тик), компилятор-сгенерированный конечный автомат вызывает `OnCompleted` на awaiter'е. Awaiter вызывает `PlayerLoopHelper.AddContinuation(timing, action, state)`, что ставит коллбэк в очередь `ContinuationQueue` для этой фазы.
...
Фаза `Update` также отслеживает глобальный счётчик кадров и периодически запускает обрезку пула объектов (каждые `VlkTask.TrimCheckInterval` кадров).
...
При следующем входе в Play Mode `Init()` срабатывает через `RuntimeInitializeOnLoadMethod` и выделяются новые очереди и runner'ы. Проверка встраивания PlayerLoop (`HasVlkTaskSystems`) предотвращает повторное добавление маркерных систем, если домен не был полностью перезагружен.
