...
Каналы создаются через статический фабричный класс `VlkTask.Channel`. Публичного конструктора нет.
...
`VlkTask`, завершающийся когда канал полностью опустошён после вызова `Complete()`. В частности:
...
|-------------|---------------|---------------------------|
| Тип возврата | `VlkTask` / `VlkTask<T>` | `ValueTask` / `ValueTask<T>` |
...
Основное отличие в том, что каналы Valkarn нативно интегрированы с `VlkTask` для ожидания без накладных расходов в сборках Unity, и путь единственного потребителя избегает аренды/возврата из пула при каждом вызове `ReadAsync`, встраивая ядро завершения непосредственно в канал.
