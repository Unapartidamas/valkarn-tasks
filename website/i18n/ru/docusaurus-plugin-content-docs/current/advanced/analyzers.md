...
- **`UnaPartidaMas.Valkarn.Tasks.SourceGen.dll`** — правила для корректности VlkTask и жизненного цикла Unity. Идентификаторы правил начинаются с `TT`.
...
Эти правила выявляют ошибки, связанные с природой единственного потребления `VlkTask` и неправильным использованием fire-and-forget.
...
`VlkTask` рассчитан на единственное потребление: после ожидания внутренний токен устаревает. Второй `await` той же переменной встретит устаревший токен и выбросит исключение. Анализатор обнаруживает, когда одна и та же локальная переменная, параметр или поле типа `VlkTask`/`VlkTask<T>` ожидается более одного раза внутри одного метода.
...
Вызов метода, возвращающего `VlkTask`, использованный как выражение-оператор — без await, без присваивания и без последующего `.Forget()` — является скрытой ошибкой. Исключения, выброшенные внутри задачи, никогда не наблюдаются, а пулируемый конечный автомат задачи никогда не возвращается в пул.

Анализатор проверяет выражения-операторы, где тип выражения разрешается в `VlkTask` или `VlkTask<T>`. Пропускает:
...
async VlkTask Bad()
{
    VlkTask task = DoWorkAsync();  // присвоено, но никогда не ожидалось
    DoOtherThing();
...
async VlkTask PollForever(CancellationToken ct)
{
    while (true)
    {
        ...
        ProcessNextItem();
        await VlkTask.Yield();       // также удовлетворяет проверке
    }
}
```
...
`[NoAutoCancel]` на `async VlkTask`-методе в `MonoBehaviour` отказывает методу в автоматической отмене жизненного цикла. Но если метод не имеет параметра `CancellationToken`, у него нет механизма для наблюдения отмены вообще, что делает атрибут бессмысленным и почти наверняка указывает на забытый параметр.
...
{
    [NoAutoCancel]
    async VlkTask Chase()      // TT014: [NoAutoCancel] без параметра CancellationToken
    {
...
Когда вы `await` Unity `Awaitable` (из `UnityEngine`) внутри `async VlkTask`-метода, Valkarn Tasks автоматически генерирует мост-адаптер через `AwaitableBridge`. Этот диагностический вывод информационный: он подтверждает, что мост активен. Никаких действий не требуется.
...
Метод `async VlkTask` (или `async VlkTask<T>`), не содержащий выражений `await` — включая `await foreach`, `await using` и `await` внутри объявлений `using` — несёт полные накладные расходы на аллокацию конечного автомата без какой-либо пользы. Компилятор всё равно генерирует конечный автомат, но поскольку точки приостановки нет, метод всегда завершается синхронно.
...
**Срабатывает на:**
```csharp
async VlkTask<int> ComputeTotal()  // TT016: нет await в теле
{
...
`[FireAndForget]` сигнализирует, что метод намеренно запускается без ожидания его результата. Применение его к методу, возвращающему `VlkTask<T>` (типизированный результат), всегда отбрасывает `T`, делая типизированный возврат бессмысленным. Метод должен возвращать `VlkTask` (void) вместо этого.

**Срабатывает на:**
```csharp
[FireAndForget]
async VlkTask<int> SendReport()   // TT017: возвращаемое значение отбрасывается
{
    await UploadAsync();
...
**Исправление — изменить тип возврата на `VlkTask`:**
```csharp
[FireAndForget]
...
Обнаружено использование типа `UniTask` (самой структуры или `UniTask<T>`). Замените его на эквивалент `VlkTask` или `VlkTask<T>`.
...
Метод возвращает тип `Awaitable` от Unity. Рассмотрите замену на `VlkTask`, который нативно интегрируется со средой выполнения Valkarn Tasks и поддерживает авто-отмену, пулирование и полный набор комбинаторов.
...
`Channel.CreateSingleConsumerUnbounded<T>()` из UniTask создаёт канал без обратного давления. Рассмотрите замену на `VlkTask.Channel.CreateBounded<T>(capacity)` для добавления обратного давления и предотвращения неограниченного роста памяти под нагрузкой.
...
// После — вариант 2: стандартный VlkTask + .Forget() на месте вызова
async VlkTask StartLoadAsync() { ... }
// Вызывается как:
...
Замените на `VlkTask.RunOnThreadPool`. API идентичен.
...
`.ToCoroutine()` был мостом UniTask для устаревших вызывающих на корутинах. Перепишите потребляющий код как метод `async VlkTask` вместо этого.
...
`UniTask.Create(Func<UniTask>)` оборачивает делегат-фабрику. Замените паттерном `VlkTask.Promise<T>` для управления завершением вручную.
...
`UniTask.Lazy<T>` и `UniTask.Defer` существовали для избежания аллокаций, когда задача могла завершиться синхронно. Valkarn Tasks имеет встроенный быстрый путь без аллокаций для синхронного случая: возврат `VlkTask.CompletedTask` или `VlkTask.FromResult(value)` никогда не выделяет память. Удалите обёртки `Lazy`/`Defer`.
