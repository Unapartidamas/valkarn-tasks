...
Jeder Kombinator und jeder Delay-Typ erstellt eine gepoolte Promise-Klasse, die `VlkTask.ISource` oder `VlkTask.ISource<T>` implementiert:

- `AsyncVlkTaskRunner<TStateMachine>` — der gepoolte Zustandsmaschinen-Runner, einer pro async-Methode (spezialisiert auf `TStateMachine`)
...
Diese Typen werden über generische Factory-Methoden instanziiert (`VlkTaskPool<T>.GetOrCreate`). Der statische Analyse-Call-Graph beginnt bei einem generischen Methodenaufruf und kann nicht zuverlässig alle konkreten `T`-Instanziierungen verfolgen, sodass ohne `link.xml` jeder dieser Typen entfernt werden kann.
...
- Jede `async VlkTask`-Methode in Ihrem Projekt erzeugt einen separaten nativen `AsyncVlkTaskRunner<TYourStateMachine>`-Typ.
- Wenn der Stripper `AsyncVlkTaskRunner<T>` entfernt, bevor alle Instanziierungen gesehen wurden, können einige async-Methoden zur Laufzeit abstürzen.
...
Der Objekt-Pool (`VlkTaskPool<T>`) enthält einen expliziten Hinweis in seinem Dokumentationskommentar:
...
- **Überlaufpfad (beliebiger Thread):** Ein Treiber-Lock-Free-Stack mit `Interlocked.CompareExchange` für Korrektheit unter gleichzeitigem Zugriff von Hintergrund-Threads (z. B. `VlkTask.RunOnThreadPool`).

Die Haupt-Thread-Identität wird in einem `volatile int`-Feld in `VlkTaskPoolShared.MainThreadId` gespeichert, das beim Start über `RuntimeInitializeOnLoadMethod` einmalig veröffentlicht wird. IL2CPP behandelt `volatile`-Felder korrekt.
...
- Generisches Sharing für Struct-`TStateMachine`-Instanziierungen gilt auf Konsolen. Jede `async VlkTask`-Methode generiert ihren eigenen nativen Typ — das ist erwartetes Verhalten und wird korrekt behandelt.
...
2. **Führen Sie einen Smoke-Test durch.** Ein minimaler Test, der `VlkTask.Delay`, `WhenAll` und eine benutzerdefinierte `VlkTaskCompletionSource` ausübt, instanziiert die am häufigsten entfernten Typen. Wenn diese drei Szenarien den Build überleben, ist der Kern intakt.
