...
हर combinator और delay type एक pooled promise class create करता है जो `VlkTask.ISource` या `VlkTask.ISource<T>` implement करता है:

- `AsyncVlkTaskRunner<TStateMachine>` — pooled state machine runner, प्रत्येक async method के लिए एक (`TStateMachine` पर specialized)
...
ये types generic factory methods (`VlkTaskPool<T>.GetOrCreate`) के माध्यम से instantiate होते हैं। Static analysis call graph एक generic method call पर start होता है और सभी concrete `T` instantiations reliably trace नहीं कर सकता, इसलिए `link.xml` के बिना इनमें से कोई भी type strip हो सकता है।
...
- आपके project में हर `async VlkTask` method एक separate `AsyncVlkTaskRunner<TYourStateMachine>` native type produce करती है।
- यदि stripper सभी instantiations देखने से पहले `AsyncVlkTaskRunner<T>` remove करता है, कुछ async methods runtime पर crash कर सकते हैं।
...
Object pool (`VlkTaskPool<T>`) के documentation comment में explicit note है:
...
- **Overflow path (any thread):** Background threads से concurrent access के तहत correctness के लिए `Interlocked.CompareExchange` के साथ Treiber lock-free stack (जैसे `VlkTask.RunOnThreadPool`)।

Main thread identity `VlkTaskPoolShared.MainThreadId` में `volatile int` field में stored है, startup पर `RuntimeInitializeOnLoadMethod` के माध्यम से once publish होती है। IL2CPP `volatile` fields को correctly handle करता है।
...
- Console platforms पर struct `TStateMachine` instantiations के लिए generic sharing apply होता है। प्रत्येक `async VlkTask` method अपना native type generate करती है — यह expected behaviour है और correctly handled है।
...
2. **Smoke test run करें।** एक minimal test जो `VlkTask.Delay`, `WhenAll`, और custom `VlkTaskCompletionSource` exercise करे, सबसे frequently stripped types instantiate करेगा। यदि वे तीन scenarios build survive करते हैं, core intact है।
