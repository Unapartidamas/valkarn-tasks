...
Cada combinador y tipo de retraso crea una clase de promesa agrupada que implementa `VlkTask.ISource` o `VlkTask.ISource<T>`:

- `AsyncVlkTaskRunner<TStateMachine>` — el runner de máquina de estados agrupado, uno por método async (especializado en `TStateMachine`)
...
Estos tipos se instancian mediante métodos de fábrica genéricos (`VlkTaskPool<T>.GetOrCreate`). El grafo de llamadas del análisis estático comienza en una llamada a método genérico y no puede rastrear de forma fiable todas las instanciaciones concretas de `T`, por lo que sin `link.xml` cualquiera de estos tipos puede ser eliminado.
...
- Cada método `async VlkTask` en tu proyecto produce un tipo nativo `AsyncVlkTaskRunner<TYourStateMachine>` separado.
- Si el eliminador quita `AsyncVlkTaskRunner<T>` antes de ver todas las instanciaciones, algunos métodos async pueden fallar en tiempo de ejecución.
...
El grupo de objetos (`VlkTaskPool<T>`) contiene una nota explícita en su comentario de documentación:
...
- **Ruta de desbordamiento (cualquier hilo):** Una pila lock-free de Treiber con `Interlocked.CompareExchange` para corrección bajo acceso concurrente desde hilos en segundo plano (por ejemplo, `VlkTask.RunOnThreadPool`).

La identidad del hilo principal se almacena en un campo `volatile int` en `VlkTaskPoolShared.MainThreadId`, publicado una vez al inicio mediante `RuntimeInitializeOnLoadMethod`. IL2CPP maneja correctamente los campos `volatile`.
...
- La compartición genérica para instanciaciones de struct `TStateMachine` aplica en consolas. Cada método `async VlkTask` genera su propio tipo nativo — este es el comportamiento esperado y se maneja correctamente.
...
2. **Ejecuta una prueba de humo.** Una prueba mínima que ejercite `VlkTask.Delay`, `WhenAll` y un `VlkTaskCompletionSource` personalizado instanciará los tipos eliminados con más frecuencia. Si esos tres escenarios superan la compilación, el núcleo está intacto.
