...
`VlkTaskSettings` est un `ScriptableObject` Unity qui contrôle le comportement à l'exécution de Valkarn Tasks — principalement le pool d'objets qui recycle les machines d'état async et les objets promise pour éviter le garbage pendant le jeu.
...
3. Nommez le fichier `VlkTaskSettings` et placez-le dans le dossier `Resources`.

Le fichier doit être nommé exactement `VlkTaskSettings` et doit résider dans un dossier nommé `Resources` n'importe où dans votre projet. L'asset est chargé à l'exécution avec `Resources.Load<VlkTaskSettings>("VlkTaskSettings")`.
...
Les paramètres de pool les plus couramment nécessaires sont également exposés directement sur `VlkTask` pour la commodité :

```csharp
int max      = VlkTask.DefaultMaxPoolSize;   // lit depuis VlkTaskSettings.Instance
int min      = VlkTask.MinPoolSize;
int interval = VlkTask.TrimCheckInterval;
```

Dans les builds non-Unity (tests, .NET autonome), `VlkTaskSettings` est une classe statique avec des propriétés mutables au lieu d'un `ScriptableObject`. Les mêmes noms de propriétés s'appliquent et peuvent être écrits directement :
...
Les fautes non observées sont toujours signalées via l'événement `VlkTask.UnobservedException` quel que soit ce paramètre.
...
Les pools s'enregistrent automatiquement lors de la première utilisation. La méthode `VlkTask.GetPoolInfo()` retourne un instantané de tous les pools actuellement enregistrés avec leur type, taille actuelle et taille maximale — c'est ce que la fenêtre Task Tracker affiche.
...
Affiche les trois paramètres de pool en direct lus depuis `VlkTask.DefaultMaxPoolSize`, `VlkTask.TrimCheckInterval` et `VlkTask.MinPoolSize`. Les valeurs ne sont affichées qu'en mode Play — en mode Édition, une note vous invite à entrer en mode Play pour voir les valeurs en direct.

L'onglet Config affiche également une référence à l'asset `VlkTaskSettings` (s'il en existe un dans `Resources`), afin que vous puissiez cliquer pour l'inspecter ou le modifier. Si aucun asset n'est trouvé, un avertissement vous dirige vers la création d'un.
...
Convertissez tout `VlkTask` ou `VlkTask<T>` en `Result` sans `try/catch` dans votre propre code :
