...
Les canaux sont créés via la classe de fabrique statique `VlkTask.Channel`. Il n'y a pas de constructeur public.
...
Un `VlkTask` qui se termine quand le canal est entièrement drainé après que `Complete()` a été appelé. Spécifiquement :
...
|----------------|-----------------|---------------------------|
| Type de retour | `VlkTask` / `VlkTask<T>` | `ValueTask` / `ValueTask<T>` |
...
La différence principale est que Valkarn Channels s'intègre nativement avec `VlkTask` pour un await sans surcharge dans les builds Unity, et le chemin consommateur unique évite une location/retour de pool à chaque appel `ReadAsync` en embarquant le cœur de completion directement dans le canal.
