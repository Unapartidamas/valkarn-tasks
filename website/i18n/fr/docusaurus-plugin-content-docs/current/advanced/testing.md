...
| Type | Objectif |
|------|---------|
| `VlkTaskTestHelper` | Initialiser et démonter le runtime Valkarn Tasks pour une session de test |
...
`VlkTaskTestHelper` est une classe utilitaire statique dans l'espace de noms `UnaPartidaMas.Valkarn.Tasks.Testing`.
...
```csharp
clock = VlkTaskTestHelper.Setup(defaultDeltaTime: 1f / 30f);  // simulation à 30 fps
```
...
```csharp
var task = VlkTask.Delay(3000);  // délai de 3 secondes
...
## Écriture de tests unitaires pour les méthodes async VlkTask

### Tests qui se terminent de manière synchrone

Certaines opérations `VlkTask` se terminent sans se suspendre — par exemple, attendre `VlkTask.CompletedTask`, `VlkTask.FromResult(value)`, ou un `VlkTaskCompletionSource` qui a été complété avant d'être attendu. Ceux-ci ne nécessitent pas d'horloge du tout et peuvent utiliser la classe interne `TestHelper` directement (interne à l'assembly de test) :
...
- `TestHelper.EnsureInitialized()` définit `VlkTaskPoolShared.MainThreadId` sur le thread courant. Cela est requis pour que les opérations de pool se dirigent vers le chemin rapide correct (non-atomique).
- `TestHelper.RunSync(VlkTask task)` affirme que la tâche est déjà complétée et appelle `GetResult()` — utile pour tester les chemins de code qui devraient se terminer de manière synchrone.
- `TestHelper.RunSync<T>(VlkTask<T> task)` fait de même et retourne la valeur résultante.
- `TestHelper.UnobservedExceptionCollector` s'abonne à `VlkTask.UnobservedException` pour la durée de sa portée, collectant toutes les exceptions non observées pour assertion.

### Tests qui nécessitent du temps

Tout test impliquant `VlkTask.Delay`, `VlkTask.Yield`, ou du code planifié à un timing PlayerLoop nécessite `VlkTaskTestHelper.Setup()` et le `TestClock` retourné.
...
public class DelayTests
{
    ...
    public void WaitAndLog_ZeroDelay_CompletesImmediately()
    {
        ...
        // VlkTask.Delay(0) retourne CompletedTask immédiatement
        Assert.IsTrue(task.IsCompleted);
    }
}
```
...
## Exemple : Tester VlkTask.Delay avec avancement de frames
...
- `VlkTaskTestHelper.Setup()` installe le `TestClock` avant chaque test ; l'attribut `[SetUp]` d'UTF l'appelle.
- `VlkTaskTestHelper.Teardown()` supprime l'horloge dans `[TearDown]`.
...
3. Si le test nécessite un contrôle du temps, utilisez `VlkTaskTestHelper.Setup()` / `Teardown()` dans `[SetUp]` / `[TearDown]`.
