...
كل مُدمِج ونوع تأخير ينشئ فئة وعود مُجمَّعة تنفِّذ `VlkTask.ISource` أو `VlkTask.ISource<T>`:

- `AsyncVlkTaskRunner<TStateMachine>` — مشغِّل آلة الحالة المُجمَّعة، واحد لكل دالة async (متخصص على `TStateMachine`)
...
هذه الأنواع تُنشأ عبر دوال مصانع جنيرية (`VlkTaskPool<T>.GetOrCreate`). يبدأ مخطط استدعاء التحليل الثابت من استدعاء دالة جنيريكية ولا يستطيع تتبع جميع إنشاءات `T` الملموسة بشكل موثوق، لذا بدون `link.xml` يمكن تجريد أي من هذه الأنواع.
...
- كل دالة `async VlkTask` في مشروعك تُنتج نوع `AsyncVlkTaskRunner<TYourStateMachine>` أصليًا منفصلاً.
- إذا أزال المجرِّد `AsyncVlkTaskRunner<T>` قبل رؤية جميع الإنشاءات، قد تتعطل بعض الدوال غير المتزامنة في وقت التشغيل.
...
تحتوي مجموعة الكائنات (`VlkTaskPool<T>`) على ملاحظة صريحة في تعليق توثيقها:
...
- **مسار الفيض (أي خيط):** كومة Treiber خالية من القفل مع `Interlocked.CompareExchange` للصحة تحت الوصول المتزامن من الخيوط الخلفية (مثل `VlkTask.RunOnThreadPool`).

هوية الخيط الرئيسي مخزَّنة في حقل `volatile int` في `VlkTaskPoolShared.MainThreadId`، يُنشَر مرة واحدة عند بدء التشغيل عبر `RuntimeInitializeOnLoadMethod`. يتعامل IL2CPP بشكل صحيح مع الحقول `volatile`.
...
- المشاركة الجنيرية لإنشاءات `TStateMachine` ذات البنية تنطبق على أجهزة الألعاب. كل دالة `async VlkTask` تُولِّد نوعها الأصلي — هذا سلوك متوقَّع ويُعالَج بشكل صحيح.
...
2. **شغِّل اختبار دخان.** اختبار بسيط يُمارس `VlkTask.Delay` و`WhenAll` ومخصص `VlkTaskCompletionSource` سيُنشئ أكثر الأنواع عرضة للتجريد. إذا نجت هذه الثلاثة سيناريوهات من البناء، فإن النواة سليمة.
