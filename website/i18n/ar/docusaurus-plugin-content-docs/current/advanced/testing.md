...
| النوع | الغرض |
|------|---------|
| `VlkTaskTestHelper` | تهيئة وإيقاف بيئة تشغيل Valkarn Tasks لجلسة اختبار |
...
`VlkTaskTestHelper` هو فئة أداة ثابتة في مساحة الاسم `UnaPartidaMas.Valkarn.Tasks.Testing`.
...
```csharp
clock = VlkTaskTestHelper.Setup(defaultDeltaTime: 1f / 30f);  // محاكاة 30 fps
```
...
```csharp
var task = VlkTask.Delay(3000);  // تأخير 3 ثوانٍ
...
## كتابة اختبارات الوحدة لدوال VlkTask غير المتزامنة

### الاختبارات التي تكتمل بشكل متزامن

بعض عمليات `VlkTask` تكتمل دون تعليق — على سبيل المثال، انتظار `VlkTask.CompletedTask` أو `VlkTask.FromResult(value)` أو `VlkTaskCompletionSource` اكتمل قبل انتظاره. هذه لا تتطلب ساعة على الإطلاق ويمكنها استخدام الفئة الداخلية `TestHelper` مباشرةً (داخلية لتجميع الاختبار):
...
- `TestHelper.EnsureInitialized()` يضبط `VlkTaskPoolShared.MainThreadId` على الخيط الحالي. هذا مطلوب حتى تتوجه عمليات المجموعة إلى المسار السريع الصحيح (غير الذري).
- `TestHelper.RunSync(VlkTask task)` يؤكد أن المهمة مكتملة بالفعل ويستدعي `GetResult()` — مفيد لاختبار مسارات الكود التي يجب أن تكتمل بشكل متزامن.
- `TestHelper.RunSync<T>(VlkTask<T> task)` يفعل نفس الشيء ويُرجع قيمة النتيجة.
- `TestHelper.UnobservedExceptionCollector` يشترك في `VlkTask.UnobservedException` طوال مدة نطاقه، جامعًا أي استثناءات غير ملاحَظة للتأكيد عليها.

### الاختبارات التي تتطلب الوقت

أي اختبار يتضمن `VlkTask.Delay` أو `VlkTask.Yield` أو الكود المجدوَل عند توقيت PlayerLoop يتطلب `VlkTaskTestHelper.Setup()` و`TestClock` المُرجَع.
...
public class DelayTests
{
    ...
    public void WaitAndLog_ZeroDelay_CompletesImmediately()
    {
        ...
        // VlkTask.Delay(0) يُرجع CompletedTask فورًا
        Assert.IsTrue(task.IsCompleted);
    }
}
```
...
## مثال: اختبار VlkTask.Delay مع تقدم الإطارات
...
- `VlkTaskTestHelper.Setup()` يثبِّت `TestClock` قبل كل اختبار؛ سمة `[SetUp]` في UTF تستدعيه.
- `VlkTaskTestHelper.Teardown()` يُزيل الساعة في `[TearDown]`.
...
| الاختبار | `../Testing/**/*.cs` | `VlkTaskTestHelper`، `TestClock` |
...
3. إذا احتاج الاختبار للتحكم في الوقت، استخدم `VlkTaskTestHelper.Setup()` / `Teardown()` في `[SetUp]` / `[TearDown]`.
