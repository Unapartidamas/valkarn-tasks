...
`VlkTaskSettings` هو Unity `ScriptableObject` يتحكم في سلوك وقت تشغيل Valkarn Tasks — بشكل رئيسي مجموعة الكائنات التي تُعيد تدوير آلات الحالة الغير المتزامنة وكائنات الوعد لتجنب القمامة أثناء اللعب.
...
3. اسم الملف `VlkTaskSettings` وضعه داخل مجلد `Resources`.

يجب تسمية الملف `VlkTaskSettings` بالضبط ويجب أن يوجد في مجلد باسم `Resources` في أي مكان في مشروعك. يُحمَّل الأصل في وقت التشغيل بـ`Resources.Load<VlkTaskSettings>("VlkTaskSettings")`.
...
مُعاملات مجموعة الموارد الأكثر حاجةً تُعرض مباشرةً على `VlkTask` للراحة:

```csharp
int max      = VlkTask.DefaultMaxPoolSize;   // يقرأ من VlkTaskSettings.Instance
int min      = VlkTask.MinPoolSize;
int interval = VlkTask.TrimCheckInterval;
```

في بنيات غير Unity (الاختبارات، .NET مستقل)، `VlkTaskSettings` هو فئة ثابتة مع خصائص قابلة للتعديل بدلًا من `ScriptableObject`. تنطبق نفس أسماء الخصائص ويمكن كتابتها مباشرةً:
...
تُبلَّغ عن الأخطاء غير الملاحظة دائمًا عبر حدث `VlkTask.UnobservedException` بغض النظر عن هذا الإعداد.
...
تسجّل مجموعات الموارد نفسها تلقائيًا عند أول استخدام. تُرجع طريقة `VlkTask.GetPoolInfo()` لقطة من جميع مجموعات الموارد المسجلة حاليًا مع نوعها وحجمها الحالي والحجم الأقصى — هذا ما تعرضه نافذة Task Tracker.
...
يُظهر مُعاملات مجموعة الموارد الثلاثة المباشرة كما تُقرأ من `VlkTask.DefaultMaxPoolSize` و`VlkTask.TrimCheckInterval` و`VlkTask.MinPoolSize`. تُظهَر القيم فقط في وضع التشغيل — في وضع التحرير تُنبّهك ملاحظة بالدخول إلى وضع التشغيل لرؤية القيم المباشرة.

يُظهر تبويب Config أيضًا مرجعًا لأصل `VlkTaskSettings` (إذا وُجد في `Resources`)، لذا يمكنك النقر للفحص أو التعديل. إذا لم يُوجد أصل، يُوجّهك تحذير لإنشاء واحد.
...
حوّل أي `VlkTask` أو `VlkTask<T>` إلى `Result` بدون `try/catch` في كودك الخاص:
