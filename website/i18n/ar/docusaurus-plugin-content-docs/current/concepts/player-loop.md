...
نظرًا لأن ValkarnTasks تتصل مباشرةً بهذه الحلقة، فإن `await VlkTask.Yield()` لا يُعطّل خيطًا — بل يُسجّل استدعاءً يستدعيه Unity عند التكتة التالية للمرحلة المختارة، ثم يعود فورًا.
...
قبل الحقن، يفحص `PlayerLoopHelper` ما إذا كان `VlkTaskUpdate` يظهر بالفعل في أي مكان في شجرة PlayerLoop الحالية. إذا ظهر، يُتخطى الحقن. هذا يجعل الحقن **فائق الأثر** — استدعاء `Init()` عدة مرات (قد يحدث في المحرر) لا يؤدي أبدًا إلى تسجيل أنظمة مكررة.
...
عندما تكتب `await VlkTask.Yield()` (أو أي شيء يتوقف لتكتة واحدة بالضبط)، تستدعي آلة الحالة المُولَّدة من المُترجم `OnCompleted` على المُنتظِر. يستدعي المُنتظِر `PlayerLoopHelper.AddContinuation(timing, action, state)`، مما يُضيف الاستدعاء في طابور `ContinuationQueue` لذلك التوقيت.
...
توقيت `Update` يتتبع أيضًا عدد الإطارات العالمي ويُطلق دوريًا تقليص مجموعة الكائنات (كل `VlkTask.TrimCheckInterval` إطار).
...
عند الدخول إلى وضع التشغيل مجددًا، تُطلق `Init()` عبر `RuntimeInitializeOnLoadMethod` وتُخصص طوابير ومُشغّلات جديدة. فحص حقن PlayerLoop (`HasVlkTaskSystems`) يمنع إدراج أنظمة العلامات مرة ثانية إذا لم يُفرَّغ النطاق بالكامل.
