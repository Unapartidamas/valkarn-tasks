...
通道通过 `VlkTask.Channel` 静态工厂类创建。没有公共构造函数。
...
在调用 `Complete()` 且通道完全耗尽后完成的 `VlkTask`。具体来说：
...
|---------|-----------------|---------------------------|
| 返回类型 | `VlkTask` / `VlkTask<T>` | `ValueTask` / `ValueTask<T>` |
| 分配（热路径） | 零（单消费者无界） | 接近零 |
...
主要区别是 Valkarn 通道与 `VlkTask` 原生集成，在 Unity 构建中实现零开销等待，并且单消费者路径通过将完成核心直接嵌入通道，在每次 `ReadAsync` 调用时避免了对象池的租用/归还。
