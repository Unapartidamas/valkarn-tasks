...
`VlkTaskSettings` 是一个 Unity `ScriptableObject`，用于控制 Valkarn Tasks 的运行时行为——主要是回收异步状态机和 promise 对象以在游戏中避免垃圾的对象池。
...
3. 将文件命名为 `VlkTaskSettings` 并放在 `Resources` 文件夹内。

文件必须恰好命名为 `VlkTaskSettings`，且必须位于项目中任何位置的名为 `Resources` 的文件夹内。资产在运行时通过 `Resources.Load<VlkTaskSettings>("VlkTaskSettings")` 加载。
...
常用的对象池参数也直接在 `VlkTask` 上公开以方便使用：

```csharp
int max      = VlkTask.DefaultMaxPoolSize;   // 从 VlkTaskSettings.Instance 读取
int min      = VlkTask.MinPoolSize;
int interval = VlkTask.TrimCheckInterval;
```

在非 Unity 构建（测试、独立 .NET）中，`VlkTaskSettings` 是一个带有可变属性的静态类而不是 `ScriptableObject`。相同的属性名适用，可以直接写入：
...
无论此设置如何，未观察到的错误始终通过 `VlkTask.UnobservedException` 事件报告。
...
对象池在首次使用时自动注册。`VlkTask.GetPoolInfo()` 方法返回所有当前已注册对象池的快照，包含它们的类型、当前大小和最大大小——这是 Task Tracker 窗口显示的内容。
...
显示从 `VlkTask.DefaultMaxPoolSize`、`VlkTask.TrimCheckInterval` 和 `VlkTask.MinPoolSize` 读取的三个实时对象池参数。值仅在播放模式下显示——在编辑模式下，一条说明指引你进入播放模式以查看实时值。

配置标签页还显示对 `VlkTaskSettings` 资产的引用（如果 `Resources` 中存在一个），你可以点击查看或修改它。如果未找到资产，会显示警告提示你创建一个。
...
将任何 `VlkTask` 或 `VlkTask<T>` 转换为 `Result`，而无需在你的代码中使用 `try/catch`：
