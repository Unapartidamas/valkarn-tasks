...
回调抛出的异常被转发到 `VlkTask.PublishUnobservedException`，而不是通过 PlayerLoop 向上传播。
...
| 计时器来源 | `VlkTask.Delay`（托管） | `NativeTimerHeap`（非托管） |
...
返回一个在给定 `World` 被销毁时自动取消的 `CancellationToken`。内部启动一个即发即弃的 `VlkTask`，在 `world.IsCreated` 为 true 时每帧调用 `VlkTask.Yield(timing)`，然后在循环退出时取消一个 `CancellationTokenSource`。
...
public partial struct AsyncLoadSystem : ISystem
{
    public void OnCreate(ref SystemState state)
    {
        ...
        // 启动异步初始化并忘记任务。
        // Forget() 将任何未处理的异常路由到 VlkTask.PublishUnobservedException。
...
- **线程安全。** `EntityManager`、`ComponentLookup` 和结构性变化仅限主线程。仅对没有 Unity 或 ECS API 调用的纯 C# 计算使用 `VlkTask.RunOnThreadPool`。
