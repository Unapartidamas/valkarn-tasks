...
- **`UnaPartidaMas.Valkarn.Tasks.SourceGen.dll`** — Regras específicas à correção do VlkTask e ao ciclo de vida do Unity. Os IDs de regra começam com `TT`.
...
Essas regras detectam bugs relacionados à natureza de consumo único do `VlkTask` e uso indevido de fire-and-forget.
...
`VlkTask` é de consumo único: uma vez aguardado, o token interno fica obsoleto. Um segundo `await` na mesma variável encontrará esse token obsoleto e lançará exceção. O analisador detecta quando a mesma variável local, parâmetro ou campo do tipo `VlkTask`/`VlkTask<T>` é aguardado mais de uma vez dentro do mesmo método.
...
Uma chamada de método que retorna `VlkTask` usada como declaração de expressão — não aguardada, não atribuída e não seguida de `.Forget()` — é um bug silencioso. Exceções lançadas dentro da task nunca são observadas, e a máquina de estados em pool da task nunca é retornada ao pool.

O analisador verifica declarações de expressão onde o tipo da expressão resolve para `VlkTask` ou `VlkTask<T>`. Ele ignora:
...
Este ID de regra é **reservado para análise de fluxo de dados futura**. Pretende detectar o padrão atribuído-mas-nunca-aguardado — armazenar um `VlkTask` em uma variável e então nunca aguardá-lo ou descartá-lo — que o TT002 não cobre porque o TT002 só examina declarações de expressão puras.
...
async VlkTask Bad()
{
    VlkTask task = DoWorkAsync();  // atribuído mas nunca aguardado
    DoOtherThing();
...
async VlkTask PollForever(CancellationToken ct)
{
    while (true)
    {
        ...
        ProcessNextItem();
        await VlkTask.Yield();       // também satisfaz a verificação
    }
}
```
...
`[NoAutoCancel]` em um método `async VlkTask` em um `MonoBehaviour` opta o método para fora do cancelamento automático de ciclo de vida. Mas se o método não tem parâmetro `CancellationToken`, não tem mecanismo para observar o cancelamento, tornando o atributo inútil e quase certamente indicando um parâmetro esquecido.
...
{
    [NoAutoCancel]
    async VlkTask Chase()      // TT014: [NoAutoCancel] mas sem parâmetro CancellationToken
    {
...
Quando você faz `await` em um `Awaitable` do Unity (de `UnityEngine`) dentro de um método `async VlkTask`, o Valkarn Tasks gera automaticamente um adaptador bridge via `AwaitableBridge`. Este diagnóstico é informacional: confirma que a bridge está em vigor. Nenhuma ação é necessária.
...
**Dispara em:**
```csharp
async VlkTask<int> ComputeTotal()  // TT016: sem await no corpo
{
...
`[FireAndForget]` sinaliza que um método é intencionalmente executado sem aguardar seu resultado. Aplicá-lo a um método que retorna `VlkTask<T>` (resultado tipado) sempre descarta `T`, tornando o retorno tipado sem sentido. O método deve retornar `VlkTask` (void) em vez disso.

**Dispara em:**
```csharp
[FireAndForget]
async VlkTask<int> SendReport()   // TT017: valor de retorno é descartado
{
    await UploadAsync();
...
**Correção — alterar o tipo de retorno para `VlkTask`:**
```csharp
[FireAndForget]
...
Um uso de um tipo `UniTask` (a struct em si ou `UniTask<T>`) foi detectado. Substitua pelo equivalente `VlkTask` ou `VlkTask<T>`.
...
Um método retorna o tipo `Awaitable` do Unity. Considere substituí-lo por `VlkTask`, que se integra nativamente com o runtime do Valkarn Tasks e suporta cancelamento automático, pooling e o conjunto completo de combinadores.
...
`Channel.CreateSingleConsumerUnbounded<T>()` do UniTask cria um canal sem backpressure. Considere substituí-lo por `VlkTask.Channel.CreateBounded<T>(capacity)` para adicionar backpressure e evitar crescimento ilimitado de memória sob carga.
...
// Depois — opção 2: VlkTask padrão + .Forget() no local de chamada
async VlkTask StartLoadAsync() { ... }
// Chamado como:
...
Substitua por `VlkTask.RunOnThreadPool`. A API é a mesma.
...
`.ToCoroutine()` era uma bridge UniTask para chamadores de coroutine legados. Reescreva o código consumidor como um método `async VlkTask` em vez disso.
...
`UniTask.Create(Func<UniTask>)` envolve um delegate factory. Substitua pelo padrão `VlkTask.Promise<T>` para conclusão controlada manualmente.
...
`UniTask.Lazy<T>` e `UniTask.Defer` existiam para evitar alocação quando uma task poderia completar de forma síncrona. O Valkarn Tasks tem um caminho rápido síncrono de zero alocação integrado: retornar `VlkTask.CompletedTask` ou `VlkTask.FromResult(value)` nunca aloca. Remova os wrappers `Lazy`/`Defer`.
