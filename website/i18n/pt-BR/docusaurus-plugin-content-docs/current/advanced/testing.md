...
| Tipo | Finalidade |
|------|-----------|
| `VlkTaskTestHelper` | Inicializar e encerrar o runtime do Valkarn Tasks para uma sessão de testes |
...
`VlkTaskTestHelper` é uma classe utilitária estática no namespace `UnaPartidaMas.Valkarn.Tasks.Testing`.
...
```csharp
clock = VlkTaskTestHelper.Setup(defaultDeltaTime: 1f / 30f);  // simulação a 30 fps
```
...
```csharp
var task = VlkTask.Delay(3000);  // delay de 3 segundos
...
## Escrevendo testes unitários para métodos async VlkTask

### Testes que completam sincronamente

Algumas operações `VlkTask` completam sem suspender — por exemplo, aguardar `VlkTask.CompletedTask`, `VlkTask.FromResult(value)`, ou um `VlkTaskCompletionSource` que foi completado antes de ser aguardado. Esses não requerem um clock e podem usar a classe interna `TestHelper` diretamente (interna à assembly de testes):
...
- `TestHelper.EnsureInitialized()` define `VlkTaskPoolShared.MainThreadId` para a thread atual. Isso é necessário para que as operações de pool sejam roteadas para o caminho rápido correto (não-atômico).
- `TestHelper.RunSync(VlkTask task)` afirma que a task já está concluída e chama `GetResult()` — útil para testar caminhos de código que devem completar sincronamente.
- `TestHelper.RunSync<T>(VlkTask<T> task)` faz o mesmo e retorna o valor do resultado.
- `TestHelper.UnobservedExceptionCollector` assina `VlkTask.UnobservedException` pelo tempo de sua existência, coletando quaisquer exceções não observadas para asserção.

### Testes que requerem tempo

Qualquer teste envolvendo `VlkTask.Delay`, `VlkTask.Yield`, ou código agendado em um timing do PlayerLoop requer `VlkTaskTestHelper.Setup()` e o `TestClock` retornado.
...
public class DelayTests
{
    ...
    public void WaitAndLog_ZeroDelay_CompletesImmediately()
    {
        ...
        // VlkTask.Delay(0) retorna CompletedTask imediatamente
        Assert.IsTrue(task.IsCompleted);
    }
}
```
...
## Exemplo: Testando VlkTask.Delay com avanço de frames
...
- `VlkTaskTestHelper.Setup()` instala o `TestClock` antes de cada teste; o atributo `[SetUp]` do UTF o chama.
- `VlkTaskTestHelper.Teardown()` remove o clock em `[TearDown]`.
...
3. Se o teste precisa de controle de tempo, use `VlkTaskTestHelper.Setup()` / `Teardown()` em `[SetUp]` / `[TearDown]`.
