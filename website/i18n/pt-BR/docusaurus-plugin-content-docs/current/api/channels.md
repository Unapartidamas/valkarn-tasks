...
Os canais são criados por meio da classe factory estática `VlkTask.Channel`. Não há construtor público.
...
Um `VlkTask` que completa quando o canal está totalmente drenado após `Complete()` ser chamado. Especificamente:
...
A diferença principal é que os Canais Valkarn se integram nativamente com `VlkTask` para await sem overhead em builds Unity, e o caminho de consumidor único evita um alugar/retornar de pool em cada chamada `ReadAsync` ao embutir o core de conclusão diretamente no canal.
