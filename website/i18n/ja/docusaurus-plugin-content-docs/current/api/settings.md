...
`VlkTaskSettings`はValkarn Tasksのランタイム動作を制御するUnityの`ScriptableObject`です — 主にガベージを避けるために非同期ステートマシンとプロミスオブジェクトをリサイクルするオブジェクトプールです。
...
3. ファイルを`VlkTaskSettings`と名前をつけて`Resources`フォルダー内に配置します。

ファイルは正確に`VlkTaskSettings`という名前で、プロジェクト内のどこかの`Resources`フォルダーに配置する必要があります。アセットは`Resources.Load<VlkTaskSettings>("VlkTaskSettings")`でランタイムにロードされます。
...
よく使用されるプールパラメーターは便宜上`VlkTask`に直接公開されています：

```csharp
int max      = VlkTask.DefaultMaxPoolSize;   // VlkTaskSettings.Instanceから読み取る
int min      = VlkTask.MinPoolSize;
int interval = VlkTask.TrimCheckInterval;
```

非Unityビルド（テスト、スタンドアロン.NET）では、`VlkTaskSettings`はScriptableObjectの代わりにミュータブルなプロパティを持つ静的クラスです。同じプロパティ名が適用され、直接書き込めます：
...
未観測のフォルトは、この設定に関わらず常に`VlkTask.UnobservedException`イベントを通じて報告されます。
...
プールは最初の使用時に自動的に登録されます。`VlkTask.GetPoolInfo()`メソッドはすべての現在登録済みプールのスナップショットを型、現在のサイズ、最大サイズとともに返します — これはTask Trackerウィンドウが表示するものです。
...
`VlkTask.DefaultMaxPoolSize`、`VlkTask.TrimCheckInterval`、`VlkTask.MinPoolSize`から読み取った3つのライブプールパラメーターを表示します。値はプレイモード中のみ表示されます — 編集モードでは、ライブ値を見るためにプレイモードに入るよう指示するメモが表示されます。

設定タブは`VlkTaskSettings`アセットへの参照も表示します（`Resources`に存在する場合）、クリックして確認または変更できます。アセットが見つからない場合は警告が表示されます。
...
`VlkTask`または`VlkTask<T>`を独自のコードでtry/catchなしに`Result`に変換します：
