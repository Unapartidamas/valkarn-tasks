...
コールバックによってスローされた例外はPlayerLoopを伝播させる代わりに`ValkarnTask.PublishUnobservedException`に転送されます。
...
| タイマーソース | `ValkarnTask.Delay`（マネージド） | `NativeTimerHeap`（アンマネージド） |
...
指定された`World`が破棄されると自動的にキャンセルされる`CancellationToken`を返します。内部的にはfire-and-forgetの`ValkarnTask`を開始し、`world.IsCreated`がtrueの間フレームごとに`ValkarnTask.Yield(timing)`を呼び出し、ループが終了すると`CancellationTokenSource`をキャンセルします。
...
public partial struct AsyncLoadSystem : ISystem
{
    public void OnCreate(ref SystemState state)
    {
        ...
        // 非同期初期化を起動してタスクを忘れる。
        // Forget()は未処理の例外をValkarnTask.PublishUnobservedExceptionにルーティングします。
...
- **スレッド安全性。** `EntityManager`、`ComponentLookup`、構造的変更はメインスレッドのみです。UnityまたはECS APIの呼び出しなしの純粋なC#計算にのみ`ValkarnTask.RunOnThreadPool`を使用してください。
