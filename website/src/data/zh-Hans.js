...
const data = {
  ...
  features: [
    { icon: '⚡', title: '零内存分配',        desc: '基于结构体的 ValkarnTask 在每条成功路径上均无堆内存压力。已完成的任务完全免费。' },
  ...
  rows: [
    ...
    },
    {
      ...
      note: {
        ...
        valkarn: 'var (tex, sfx, data) = await ValkarnTask.WhenAll(...)——最多 8 个类型化结果。WhenAny 返回第一个完成的结果及其索引。',
      },
    },
    {
      feature: '异常静默吞噬',
      ...
      note: {
        ...
        valkarn:  'ValkarnTaskSettings.UnobservedExceptionHandler 可由用户配置。默认值：记录到 Debug.LogException，确保任何异常都不会被静默。',
      },
    },
  ],
